...
/**
 * 类ThreadPoolTaskExecutor.java的实现描述: 使用了扩展的ThreadPoolExecutor
 * 
 * @author liulin 2012-2-9 下午05:22:03
 */
/**
 * JavaBean that allows for configuring a JDK 1.5 {@link java.util.concurrent.ThreadPoolExecutor} in bean style (through
 * its "corePoolSize", "maxPoolSize", "keepAliveSeconds", "queueCapacity" properties), exposing it as a Spring
 * {@link org.springframework.core.task.TaskExecutor}. This is an alternative to configuring a ThreadPoolExecutor
 * instance directly using constructor injection, with a separate {@link ConcurrentTaskExecutor} adapter wrapping it.
 * <p>
 * For any custom needs, in particular for defining a {@link java.util.concurrent.ScheduledThreadPoolExecutor}, it is
 * recommended to use a straight definition of the Executor instance or a factory method definition that points to the
 * JDK 1.5 {@link java.util.concurrent.Executors} class. To expose such a raw Executor as a Spring
 ...
 * the JDK 1.5 {@link java.util.concurrent.Executor} interface, with the former being the primary interface, the other
 * just serving as secondary convenience. For this reason, the exception handling follows the TaskExecutor contract
...
public class ThreadPoolTaskExecutor extends CustomizableThreadFactory implements SchedulingTaskExecutor, Executor, BeanNameAware, InitializingBean, DisposableBean {
    ...
    protected final Logger           logger                           = Logger.getLogger("threadPoolExceptionLogger");
    ...
    public void initialize() {
        ...
        this.threadPoolExecutor = new CustomThreadPoolExecutor(this.corePoolSize, this.maxPoolSize,
    ...
    public Future<Object> submit(Callable<Object> taskCallable) {
        ExecutorService executor = getThreadPoolExecutor();
        return executor.submit(taskCallable);
    }
